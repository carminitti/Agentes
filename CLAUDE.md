...
| Version | Changes |
|---|---|
| v1.47.0 | 2 novos executores (`executor-pytest`, `executor-observabilidade`). Roteamento completo para SOAP/Newman/SSE. `classifier-testes` com 5 novos tipos. `environment-health-check` (auth/DB/fila), `flaky-test-detector` pós-execução e `notification-dispatcher` automático integrados ao orquestrador. `coverage-gap-detector` no `qa-pipeline`. Fast Mode infere soap/sse/newman. `gherkin-validator` e `test-data-factory` com hooks automáticos. Sparkline SVG de tendência histórica no reporter. `agent_version` corrigido para 1.47.0. |
| v1.45.0 | `orquestrador-qa` — Fast Mode redesenhado: pede os casos de teste diretamente na primeira pergunta, infere tipo/URL/auth dos steps automaticamente, pergunta ao usuário apenas o que não pôde ser inferido (no máximo 1 mensagem combinada). Etapa 0 e perguntas obrigatórias da Etapa 2 bypassadas em Fast Mode via defaults e inferência. Reduz de 8+ rounds para 3–4 antes do início da execução. |
| v1.44.13 | `install.ps1` — agents em `~/.claude/agents/` funcionam como subagentes mas não como slash commands; script agora também cria `~/.claude/skills/<nome>/SKILL.md` para os 4 agents user-facing (`orquestrador-qa`, `qa-pipeline`, `revisor`, `consulta-treinamento`), habilitando `/orquestrador-qa` e demais slash commands sem reinicialização. |
